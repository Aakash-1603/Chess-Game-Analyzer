...
def load_and_process_data():
    """Load and process chess data with optimizations."""
    # Reduce sample size for faster loading
    n_games = 500  # Reduced from 1000
    
    # Use more efficient random generation
    np.random.seed(42)
    
    # Pre-calculate arrays for better performance
    results = np.random.choice(['1-0', '0-1', '1/2-1/2'], n_games, p=[0.45, 0.38, 0.17])
    white_elos = np.random.normal(1500, 200, n_games).astype(int)
    black_elos = np.random.normal(1500, 200, n_games).astype(int)
    
    # Simplified moves generation (much faster)
    move_counts = np.random.randint(20, 80, n_games)
    ...
    data = {
        ...
        'num_moves': move_counts  # Pre-calculated instead of processing strings
    }
...
def main():
    try:
        ...
        status_text.text('Loading chess data...')
        ...
        if len(filtered_df) == 0:
            st.warning("No games found with current filters. Please adjust your selection.")
            return
        ...
        with tab3:
            ...
            with col2:
                ...
                if not popular_openings.empty:
                    ...
                    st.info("No openings with 10+ games found in current filter.")
        ...
        st.info("Please try refreshing the page. If the issue persists, check your data and dependencies.")
        st.stop()
